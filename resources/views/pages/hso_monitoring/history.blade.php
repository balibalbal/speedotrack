                ...
                <div class="card-header header-elements d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="card-title mb-0">
                            Tracking Perjalanan Kendaraan HSO
                        </h5>
        ...
        $('#trackButton').click(function(event) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var geofenceMapData = response.geofenceMapData;
                    console.log(response);

                    if (!Array.isArray(data) || !Array.isArray(geofenceMapData)) {
                    ...
                    var coordinates = data.map(function(item) {
                        if (item.latitude !== undefined && item.longitude !== undefined) {
                            return [parseFloat(item.latitude), parseFloat(item.latitude)];
                        } else {
                    ...
                    geofenceMapData.forEach(function(item) {
                        if (item.type == 1) {
                            // Extract latitude and longitude from the latlong field
                            var [lat, lng] = item.latlong.split(',').map(coord => parseFloat(coord.trim()));

                            // Create a circle using Leaflet's L.circle
                            var circle = L.circle([lat, lng], {
                                color: 'red',
                                radius: item.radius // Set radius from the radius field
                            }).addTo(map);

                            // Add a click event to display information
                            circle.on('click', function(e) {
                                var info = "<b>Nama:</b> " + item.name;
                                L.popup()
                                    .setLatLng(e.latlng)
                                    .setContent(info)
                                    .openOn(map);
                            ...
                            // Handle other types (e.g., polygons)
                            var coordinates = item.latlong.replace('(', '').replace(')', '');
